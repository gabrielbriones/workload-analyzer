...
class TestISSClient:
    ...
    async def test_get_jobs_with_filters(self, mock_settings):
        ...
        with patch('aiohttp.ClientSession.post') as mock_post, \
            ...
            # The URL is built using iss_base_url + /v1/ + endpoint
            expected_url = f"{mock_settings.iss_base_url}/v1/jobs"
    ...
    async def test_get_job_success(self, mock_settings, sample_job_data):
        ...
        with patch('aiohttp.ClientSession.post') as mock_post, \
            ...
            call_args = mock_request.call_args
            # The URL is built using iss_base_url + /v1/ + endpoint
            expected_url = f"{mock_settings.iss_base_url}/v1/jobs/job/{job_id}"
            assert call_args.kwargs['url'] == expected_url
